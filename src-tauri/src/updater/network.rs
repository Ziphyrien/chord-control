...
fn sources(address: &Url) -> Vec<Url> {
    ...
    if public_release {
        let mirrors: Vec<String> = serde_json::from_str(include_str!("../../../shared/github-mirrors.json"))
            .expect("Invalid bundled GitHub mirrors");
        urls.extend(mirrors.iter().filter_map(|prefix| Url::parse(&format!("{prefix}{address}")).ok()));
    }
    urls
}
...
pub(super) async fn check(app: &AppHandle) -> Result<Option<Update>, String> {
    let endpoints: Vec<Url> = serde_json::from_value(
        app.config().plugins.0.get("updater").ok_or("缺少更新配置")?["endpoints"].clone(),
    ).map_err(|e| e.to_string())?;
    let mut checks = FuturesUnordered::new();
    for endpoint in endpoints.iter().flat_map(sources) {
        let updater = app.updater_builder()
            .endpoints(vec![endpoint.clone()]).map_err(|e| e.to_string())?
            .timeout(Duration::from_secs(12))
            .configure_client(|client| client.https_only(true).connect_timeout(Duration::from_secs(5)))
            .build().map_err(|e| e.to_string())?;
        checks.push(async move {
            let result = tokio::time::timeout(Duration::from_secs(15), updater.check()).await
...
async fn probe(client: &Client, url: &Url) -> Result<(), String> {
    let mut response = client.get(url.clone()).header("Range", "bytes=0-65535")
        .send().await.map_err(|e| e.to_string())?;
    if !matches!(response.status().as_u16(), 200 | 206)
        || response.content_length().is_some_and(|size| size > MAX_INSTALLER)
    ...
    while received < PROBE_BYTES {
        let Some(chunk) = response.chunk().await.map_err(|e| e.to_string())? else { break };
        if prefix.len() < 2 { prefix.extend(chunk.iter().take(2 - prefix.len())); }
        received += chunk.len();
        if prefix.len() == 2 && prefix != b"MZ" { return Err("来源未返回 Windows 安装包".into()); }
    }
    if prefix != b"MZ" { return Err("安装包为空或不完整".into()); }
    Ok(())
}

pub(super) async fn download(update: &Update) -> Result<Vec<u8>, String> {
    let client = Client::builder().https_only(true).user_agent("chord-control")
        .connect_timeout(Duration::from_secs(5)).timeout(Duration::from_secs(12))
        .build().map_err(|e| e.to_string())?;
    let mut remaining = sources(&update.download_url);
    let mut errors = Vec::new();
    while !remaining.is_empty() {
        let mut probes: FuturesUnordered<_> = remaining.iter().cloned().map(|url| {
            let client = &client;
            async move { let result = probe(client, &url).await; (url, result) }
        }).collect();
        let mut winner = None;
        while let Some((url, result)) = probes.next().await {
            match result {
                Ok(()) => { winner = Some(url); break; }
                Err(error) => {
        ...
        let mut size = 0usize;
        let mut transfer = Box::pin(candidate.download(|chunk, total| {
...
mod tests {
    ...
    fn mirrors_only_receive_public_release_urls() {
        for address in ["https://github.com/a/b/releases/latest/download/latest.json",
            "https://github.com/a/b/releases/download/app-v1/setup.exe"] {
            assert_eq!(sources(&Url::parse(address).unwrap()).len(), 4);
        }
        for address in ["https://example.org/setup.exe", "https://github.com/a/b/releases/download/v1/a?token=secret",
            "https://user:secret@github.com/a/b/releases/download/v1/a", "http://github.com/a/b/releases/download/v1/a",
            "https://github.com.evil.test/a/b/releases/download/v1/a", "https://github.com/a/b/releases/download/v1/a#secret"] {
